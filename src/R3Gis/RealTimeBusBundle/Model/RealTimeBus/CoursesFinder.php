...
SELECT
    ...
    frt_start + COALESCE(travel_time, 0) + COALESCE(delay_sec, 0) AS arrives_in,	
    mta.tagesart_text,
    fahrtart_nr,
    li_r,
    li_g,
...
WHERE ort_nr={$stopId['ort_nr']}
    ...
    AND frt_start + COALESCE(travel_time, 0) + COALESCE(delay_sec, 0) > EXTRACT(EPOCH FROM (now()-current_date))
